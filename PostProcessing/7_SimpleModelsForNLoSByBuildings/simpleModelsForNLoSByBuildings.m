...
% Set this to be true input to generate debug plots.
...
%plot3k([dsInMFromTx, yPlaneZerosPadding, pathLossesDb], 'Marker', {'.', 10});
hMeas = plot3(dsInMFromTx, yPlaneZerosPadding, pathLossesDb, 'b.', ...
    'MarkerSize', 10);
% Ref ITU model results.
yPlaneOnesPadding = ones(length(dsInMFromTx),1);
ituPatchXs = [dsInMFromTx; dsInMFromTx(end:-1:1)];
ituPatchYs = [yPlaneOnesPadding; yPlaneOnesPadding];
ituRefBottomZs = [pathLossInDbGaussianRef.pathLossInDbMean]' ...
    - 3.*[pathLossInDbGaussianRef.pathLossInDbVar]';
ituPatchZs = [[pathLossInDbGaussianRef.pathLossInDbMean]' ...
    + 3.*[pathLossInDbGaussianRef.pathLossInDbVar]'; ...
...
hMean = plot3(dsInMFromTx, yPlaneOnesPadding, ...
    [pathLossInDbGaussianKed.pathLossInDbMean]', 'k-', 'LineWidth', 1);
h3Sigma = plot3(dsInMFromTx, yPlaneOnesPadding, ...
    [pathLossInDbGaussianKed.pathLossInDbMean]' ...
    + 3.*[pathLossInDbGaussianKed.pathLossInDbVar]', '-.', ...
    'Color', ones(1,3)*0.5, 'LineWidth', 1);
plot3(dsInMFromTx, yPlaneOnesPadding, ...
    [pathLossInDbGaussianKed.pathLossInDbMean]' ...
    - 3.*[pathLossInDbGaussianKed.pathLossInDbVar]', '-.', ...
    'Color', ones(1,3)*0.5, 'LineWidth', 1);
...
text(260, 0, 167,...
    ...
    ['  ITU with KED Mean ', num2str(rmseAggKed, '%.2f'), ' dB']}, ...
    'Rotation', 0, ...
