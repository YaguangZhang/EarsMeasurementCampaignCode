% GENMOREPLOTSFORCONTI Generate more PDP plots for the continuous cases and
% save them as .png figures.
%
% This script is mainly for investigating the multipath effects of the area
% blocked by the Rickover Hall.
%
% We will process all the data collected and duplicate the folder structure
% for them into a "PostProcessingResults/SummaryReport/morePlotsForConti"
% folder under the shared Google Drive folder.
%
...
ABS_PATH_TO_SAVE_PLOTS = fullfile(ABS_PATH_TO_EARS_SHARED_FOLDER, ...
    'PostProcessingResults', 'SummaryReport', 'morePlotsForConti');
...
% one "Series_xx" folder. Note that for continuous tracks, only one (quite
% long) recording was abtained, so we set numMeasToPlotPerSeries to be 1.
numMeasToPlotPerSeries = 1;

%% Before Generating the Plots

disp(' ---------------------- ')
disp('  genMorePlotsForConti')
disp(' ---------------------- ')
        ...
        for idxDir = 1:length(dirsToProcess)
            if dirsToProcess(idxDir).isdir
                ...
                if(length(idxSeriesTokens)==1)
                    ...
                    % Ignore measurements that will not be used. We will
                    % load the last few measurements.
                    signalFilteredLogs = signalFilteredLogs(end+1-min( ...
                        [numMeasToPlotPerSeries; length(signalFilteredLogs)]):end);
                    % Load the signal samples. The integer countSam is to
                    % limit the number samples to load.
                    countSam = 1.7.*1.04.*(10^6); % ~ 3s of the signal.
                    seriesSignalFiltered = arrayfun(...
                        @(log) readComplexBinaryFromEnd(log.name, countSam), ...
                        signalFilteredLogs, ...
                        'UniformOutput', false);
                    try
                        seriesSignal = arrayfun(...
                            @(log) readComplexBinaryFromEnd(...
                            regexprep(log.name, '_filtered',''), countSam),...
                            signalFilteredLogs, ...
                            'UniformOutput', false);
                    catch
                        warning('Unable to load sig!')
                    end
                    if (length(seriesSignalFiltered)<numMeasToPlotPerSeries)
                        warning(['#', num2str(idxPar), ...
                    ...
                    if ~isempty(seriesSignalFiltered)
                        ...
                        for idxSignalFiles = 1:length(seriesSignalFiltered)
                            close all;
                            [~, seriesParentDirName] = fileparts(allSeriesParentDirs(idxPar).name);
                            figureSupTitle = [seriesParentDirName, ': Series ', num2str(idxSeries), ...
                                ' - ', num2str(idxSignalFiles)];
                            % For saving the plots.
                            nameFileToSave = [seriesParentDirName, '_oneSigPerMeas_series_', ...
                                num2str(idxSeries), '_meas_', num2str(idxSignalFiles)];
                            pathFileToSave = fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
                                nameFileToSave);
                            % We will plot the tallest bump of the last
                            % 3000 samples for each recording.
                            ...
                            try
                                ...
                                warning([nameFileToSave, ': Unable to generate PDP plot for sig!'])
                            end
                        end
                    end
                end
