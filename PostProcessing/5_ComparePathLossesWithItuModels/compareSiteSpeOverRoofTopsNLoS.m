...
if FLAG_MANUAL_INSPECTION
    ...
    for idx = 1:length(validRelPathsSerNLoSUnique)
        ...
        disp([curRelPath, ', latM = ', num2str(curMeasRec(5)), ...
    ...
    for idx = 1:numSitesNLoS
        if isgraphics(hSite) && isvalid(hSite)
            delete(hSite);
        end
        curRelPath = validRelPathsSerNLoSUnique{idx};
        idxMeasRec = find(strcmp(validRelPathsSerNLoS, curRelPath), 1);
        curMeasRec = validPathLossesWithValidGpsInfoNLoS(idxMeasRec, :);
        hSite = plot(curMeasRec(6), curMeasRec(5), 'r*');
...
for idx = 1:numResults
    ...
    rmses(idx) = sqrt(mean(...
        ( LNLoS1s(idx) - validPathLossesWithValidGpsNLoS(curIndices) ).^2 ...
        ));
end
...
% roof-tops.
gamma0NLoS = 2.3;
[alphaNLoS, bettaNLoS, ABGWithGivenGModFctNLoS] = fitAlphaBetaWithGivenGamma(...
    siteDistsFromTxNLoS, ...
...
[rmseCloNLoS, distsCloNLoS] = computeRmseOverDist( ...
    siteDistsFromTxNLoS, ...
    closeInModFctNLoS(siteDistsFromTxNLoS), ...
    validPathLossesWithValidGpsNLoS);

[rmseABGNLoS, distsABGNLoS] = computeRmseOverDist( ...
    siteDistsFromTxNLoS, ...
...
save(absPathToLoadLoSResults, ...
    ...
    'alphaNLoS', 'bettaNLoS', 'gamma0NLoS', 'ABGWithGivenGModFctNLoS', ...
    'LNLoS1sDInM', 'LNLoS1s', ...
...
% ITU model results.
yPlaneZeroPadding = zeros(length(LNLoS1sDInM),1);
hItuNLoS = plot3(LNLoS1sDInM, yPlaneZeroPadding, LNLoS1s, 'k*');
