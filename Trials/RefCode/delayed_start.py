...
class delayed_start():
	def __init__(self, usrp, delay=0, pps=True, base_time=None, offset=0, callbacks=[], gpsdo=False):
		#print "Channel Gain is: ", chann_gain		
		...
		if gpsdo:
			...
			formatted_time_first = formatted_base_time = datetime.datetime.fromtimestamp(time_first).strftime('%Y-%m-%d %H:%M:%S')
		else:
			time_first = time.time()
		time_first_int = int(time_first)
		print "Beginning sync at: ", formatted_time_first
		...
		formatted_base_time = datetime.datetime.fromtimestamp(time_first).strftime('%Y-%m-%d %H:%M:%S')
		
		if base_time is None:
			base_time = time_next_int_plus_1
		
		future_start = base_time + delay
		formatted_future_start = datetime.datetime.fromtimestamp(future_start).strftime('%Y-%m-%d %H:%M:%S')
		...
		if pps:
			...
			formatted_time_next_int_plus_1 = datetime.datetime.fromtimestamp(time_next_int_plus_1).strftime('%Y-%m-%d %H:%M:%S')
			print "USRP time set NOW: ", formatted_time_next_int_plus_1
		...
		#print "USRP start time set:", formatted_future_start
		usrp.start()
		#print "USRP 'started'..."
		print "USRP starting at:", formatted_future_start

def main(usrp):
	current_time = usrp.get_mboard_sensor("gps_time")
	locked = usrp.get_mboard_sensor("gps_locked")
	gpgga_string = usrp.get_mboard_sensor("gps_gpgga")
	formatted_current_time = datetime.datetime.fromtimestamp(current_time).strftime('%Y-%m-%d %H:%M:%S')
	print "Current Time is: ", formatted_current_time
	#print "Channel Gain is: ", chann_gain
	print "GPS Locked Condition is: ", locked
	print "Current GPS Position is: ", gpgga_string
	return 0
